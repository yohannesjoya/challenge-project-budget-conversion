const db = require('./db')
const getExchangeRateAPI = require('./utils')
...
async function insertBudget (req, res) {
  try {
    ...
    await db.executeQuery(insertQuery, record)
...
async function updateBudget (req, res) {
  try {
    ...
    await db.executeQuery(updateQuery, record)
    return res.status(201).json({ success: true })
  } catch (error) {
    return res.status(500).json({ success: false, error: 'Internal Server Error' })
  }
...
async function convertCurrenciestoTTD (req, res) {
  try {
    ...
    if (!latestTtdRate) {
      return res.status(200).json({ success: false, error: 'No exchange rate available now, try again later' })
    }

    results.forEach((data) => {
      data.finalBudgetTtd =
        parseFloat((data.finalBudgetUsd * latestTtdRate).toFixed(2))
    })
    return res.status(200).json({ success: true, data: results })
  } catch (error) {
    return res.status(500).json({ success: false, error: 'Internal Server Error' })
