...
function validateBudgetDto (data) {
  const errorList = []
  if (!data.projectName || !validator.isLength(data.projectName, { min: 1 })) {
  ...
  if (!data.currency || !validator.isLength(data.currency, { min: 1 })) {
    errorList.push('Invalid or missing currency')
  }
  if (!data.initialBudgetLocal || !validator.isFloat(String(data.initialBudgetLocal))) {
    errorList.push('Invalid or missing initialBudgetLocal')
  }
  if (!data.budgetUsd || !validator.isFloat(String(data.budgetUsd))) {
    errorList.push('Invalid or missing budgetUsd')
  }
  if (!data.initialScheduleEstimateMonths || !validator.isFloat(String(data.initialScheduleEstimateMonths))) {
    errorList.push('Invalid or missing initialScheduleEstimateMonths')
  }
  if (!data.adjustedScheduleEstimateMonths || !validator.isFloat(String(data.adjustedScheduleEstimateMonths))) {
    errorList.push('Invalid or missing adjustedScheduleEstimateMonths')
  }
  if (!data.contingencyRate || !validator.isFloat(String(data.contingencyRate))) {
    errorList.push('Invalid or missing contingencyRate')
  }
  if (!data.escalationRate || !validator.isFloat(String(data.escalationRate))) {
    errorList.push('Invalid or missing escalationRate')
  }
  if (!data.finalBudgetUsd || !validator.isFloat(String(data.finalBudgetUsd))) {
...
function validateCreateBudgetDto (data) {
  ...
  if (!data.projectId || !validator.isInt(String(data.projectId))) {
